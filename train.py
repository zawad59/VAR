...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, ld_train, trainer):
    ...
    for it, (inp, label) in enumerate(ld_train):
        inp = inp.to(args.device, non_blocking=True).float()  # Convert input to Float
        label_B = label.to(args.device, non_blocking=True).long()  # Convert label to LongTensor
        
        with torch.amp.autocast("cuda"):
            trainer.train_step(
                it=it, g_it=ep * len(ld_train) + it, stepping=True,
                metric_lg=me, tb_lg=None, inp_B3HW=inp, label_B=label_B,
